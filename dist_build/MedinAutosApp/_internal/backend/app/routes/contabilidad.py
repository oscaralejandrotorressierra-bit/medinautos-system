...
def _generar_pdf_nomina(liquidacion, mecanico, detalles, metodo_pago):
    ...
    header = Table([
        ...
        [Paragraph(f"<b>Liquidacion:</b> #{liquidacion.id}", styles["Normal"]),
         Paragraph(f"<b>Fecha:</b> {liquidacion.fecha_creacion.strftime('%Y-%m-%d')}", styles["Normal"])]
    ], colWidths=[120, 380])
    ...
    info = Table([
        ...
        [f"Mecanico: {nombre_mecanico}", f"Documento: {documento}"],
    ...
    story.append(Paragraph("Firma mecanico: ____________________", styles["Normal"]))
...
def actualizar_estado_liquidacion(
    ...
    if estado == "pagado":
        ...
        movimiento = MovimientoCaja(
            caja_id=caja.id,
            tipo="egreso",
            concepto=f"Pago nomina mecanico {nombre_mecanico}",
