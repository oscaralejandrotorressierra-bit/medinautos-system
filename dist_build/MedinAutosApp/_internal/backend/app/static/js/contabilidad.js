...
async function cargarMecanicos() {
    ...
    try {
        ...
        if (!response.ok) {
            const detalle = await obtenerDetalleError(response);
            throw new Error(detalle || "No se pudieron cargar los mecanicos.");
        }
        ...
        activarSelectBuscable(select, "Buscar mecanico...");
    } catch (error) {
...
async function onCrearLiquidacion(event) {
    ...
    if (!payload.mecanico_id || !payload.fecha_inicio || !payload.fecha_fin) {
        mostrarError("Liquidaciones", "Completa mecanico y fechas.");
        return;
    }
